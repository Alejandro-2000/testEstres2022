...
@section("content")
    ...
    <table class="table">
        ...
        <tbody>
        @foreach($datos_programa_estudio as $programaestudio)
            <tr>

                <th scope="row">{{$loop->index+1}}</th>
                <td>{{$programaestudio->descripcion_carrera}}</td>
                <td>
                <td>
                    {{--boton para editar primero debe conocer cual es el id que se desea editar entrando al metodo edit--}}
                    <a href="{{route("programaestudio.edit",$programaestudio->id_programa)}}" class="btn btn-sm btn-success"><i class="far fa-edit"></i></a>
                </td>
                <td>
                    {{--boton para editar primero debe conocer cual es el id que se desea editar entrando al metodo edit--}}
                    <a href="{{route("programaestudio.show",$programaestudio->id_programa)}}" class="btn btn-danger btn-sm"><i class="fas fa-trash-alt"></i></a>
                </td>
            </tr>
        @endforeach
    </table>
    {{--incluir las vistas donde estan los datos--}}
    @include("programaestudio.create")

    @if(session("modal_edit"))
        @include("programaestudio.edit")
    @endif

    @if(session("modal_delete"))
        @include("programaestudio.delete")
    @endif
